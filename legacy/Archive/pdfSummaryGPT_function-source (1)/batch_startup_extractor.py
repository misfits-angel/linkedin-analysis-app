"""
Batch Startup Information Extractor
Processes all PDFs in a Google Drive folder and extracts:
- File name
- Startup name
- Founder names

Can run as standalone script (no Pub/Sub needed)
"""
...
from common import authenticate_google_drive, download_text_file
...
def extract_startup_info_with_rules(content: str, file_name: str) -> Dict:
    """
    Try to extract startup name and founder names using regex patterns and heuristics.
...
def extract_startup_info(content: str, file_name: str, use_gemini_fallback: bool = True) -> Dict:
    """
...
def get_all_pdfs_from_folder(drive_service, folder_id: str) -> List[Dict]:
    """
...
def process_all_pdfs_in_folder(folder_id: str, use_gemini: bool = False, output_csv: str = None):
    """
    Process all PDFs in a Google Drive folder and export to CSV.
    
    Args:
        folder_id: Google Drive folder ID containing PDFs
        use_gemini: If True, use Gemini API as fallback for incomplete extractions
        output_csv: Output CSV file path (default: startup_founders_data.csv in temp)
    
    Returns:
        Path to the generated CSV file and list of extracted data
    """
    print("\n" + "="*60)
    print("🚀 BATCH STARTUP EXTRACTOR")
    print("="*60)
    ...
    # Get all PDFs
    pdf_files = get_all_pdfs_from_folder(drive_service, folder_id)
    ...
    # Set output CSV path
    if not output_csv:
        output_csv = os.path.join(tempfile.gettempdir(), "startup_founders_data.csv")
    ...
    for idx, pdf_file in enumerate(pdf_files, 1):
        ...
        try:
            ...
            content = extract_text_from_pdf_with_vision(
                text_path,
                bucket_name="misfits-production.appspot.com"
            )
            
            # Extract startup info
            result = extract_startup_info(content, file_name, use_gemini_fallback=use_gemini)
...
if __name__ == "__main__":
    ...
    # Usage: python batch_startup_extractor.py <FOLDER_ID> [--gemini] [--output CSV_PATH]
    
    if len(sys.argv) < 2:
        print("\n❌ Usage: python batch_startup_extractor.py <FOLDER_ID> [--gemini] [--output CSV_PATH]\n")
        print("Example:")
        print("  python batch_startup_extractor.py 1a2b3c4d5e6f")
        print("  python batch_startup_extractor.py 1a2b3c4d5e6f --gemini")
        print("  python batch_startup_extractor.py 1a2b3c4d5e6f --output /path/to/output.csv\n")
        sys.exit(1)
