...
export interface NarrativeInsightsResult {
  insights: string[]
}
 ...
 */
function prepareCondensedPosts(postsData: any[], limit: number = 50): PostData[] {
  const condensedPosts: PostData[] = []
 ...
 */
export async function generateNarrativeInsights(postsData: any[]): Promise<NarrativeInsightsResult> {
  checkAPIKey()

  const condensedPosts = prepareCondensedPosts(postsData, 50)
  
  const prompt = `
You are a LinkedIn content strategist analyzing a user's posting patterns to provide actionable insights.

Analyze these ${condensedPosts.length} LinkedIn posts and provide narrative insights about:
1. Content themes and topics
2. Engagement drivers
3. Areas for improvement

Focus on actionable, specific insights that can help improve their LinkedIn presence.
 ...
 */
export async function analyzeTopicsWithLLM(postsData: any[]): Promise<TopicAnalysisResult> {
  checkAPIKey()
 ...
 */
export async function evaluatePostsWithLLM(postsData: any[]): Promise<PostEvaluationResult> {
  checkAPIKey()
 ...
 */
export async function analyzePositioningWithLLM(postsData: any[]): Promise<PositioningAnalysisResult> {
  checkAPIKey()
