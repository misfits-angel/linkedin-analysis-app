...
interface PostQualityEvaluationProps {
  postsData: any[]
}
...
export default function PostQualityEvaluation({ postsData }: PostQualityEvaluationProps) {
  const [evaluation, setEvaluation] = useState<PostEvaluationResult | null>(null)
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {Object.entries(rubric_breakdown).map(([key, score]) => {
                const maxScore = maxScores[key as keyof typeof maxScores] || 10
                const percentage = (score / maxScore) * 100
                const label = rubricLabels[key as keyof typeof rubricLabels] || key.replace('_', ' ')
                const description = parameterDescriptions[key as keyof typeof parameterDescriptions] || ''
