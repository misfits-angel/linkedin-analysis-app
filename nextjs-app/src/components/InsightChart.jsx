...
export default function InsightChart({ data, type, title }) {
    ...
    switch (type) {
      case 'topics':
        ...
        return {
          type: 'bar',
          data: {
            labels: data.topics.slice(0, 5).map((topic) => topic.topic),
            datasets: [
              {
                label: 'Posts',
                data: data.topics.slice(0, 5).map((topic) => topic.frequency),
                backgroundColor: [
